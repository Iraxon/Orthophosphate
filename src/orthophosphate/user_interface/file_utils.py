...
def getFilePath(prompt):
    ...
    file_path = filedialog.askopenfilename(filetypes=[("MIDI Files", "*.txt *.mcdp")])
...
def createFile(path):
    try:
        with open(
            path, "x"
        ) as f:  # Using a with block is safer than open() and then close()
            # because it ensures close() is always executed even if there are errors
            pass
    except (
        FileExistsError
    ):  # "x" mode for opening raises this error if the file already exists
        pass
